...
gr::Button::Button(const sf::Vector2f& position, const sf::Vector2f& size,
                   const std::string& text, const sf::Color& buttonColor,
                   const sf::Color& textColor, const sf::Font& font, bool b)
    : pos_{position}, size_{size} {
  show_ = b;
  ...
  buttonText_.setPosition(
      position.x + size.x / 2 - buttonText_.getLocalBounds().width / 2,
...
void gr::Button::hide() { show_ = false; }
void gr::Button::show() { show_ = true; }
bool gr::Button::isVisible() { return show_; }

void gr::Button::setText(std::string s) {
  buttonText_.setString(s);
  buttonText_.setPosition(
      pos_.x + size_.x / 2 - buttonText_.getLocalBounds().width / 2,
