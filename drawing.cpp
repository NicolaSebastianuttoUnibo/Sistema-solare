#include "Graphic.hpp"
     ...
     void gr::drawing::drawArrow(sf::Vector2f start, sf::Vector2f  tip,
                          sf::Vector2f direction, const sf::Color &c, sf::RenderWindow& window) {
        ...
        sf::Vertex line3[] = {
            sf::Vertex( tip, c),
            sf::Vertex(secondvector, c)
        };
        window.draw(line3, 2, sf::Lines);
    }


      void gr::drawing::drawPlanet(const G::PlanetState &planet, sf::Vector2f translate, const sf::Color *color, sf::RenderWindow& window,  sf::FloatRect& visibleArea, bool boolean){
sf::CircleShape circle(0.f);
        float r = planet.r;


sf::FloatRect planetBounds(planet.x + translate.x - r, planet.y + translate.y - r,
                                   2 * r, 2 * r);
...
if(boolean){
   if(color!=nullptr){drawArrow(start, end, dir, *color ,window);}
   else{drawArrow(start, end, dir, sf::Color::White,window);}
}
