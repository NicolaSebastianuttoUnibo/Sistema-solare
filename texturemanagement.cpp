#include "Graphic.hpp"
 ...
 gr::TM::TM(const std::string& str, U::FileUniverse* u) : u_{u}, s_{str} { loadAndAssignTextures(s_, u_); }
...
/////CREATING TEXTURES' VECTOR
   if(texture_name_.size()==0){////creating the vector with the textures' name and loading textures only one time  
...
if(texture_vector_.size()!=texture_name_.size()){
  throw std::runtime_error("Not all textures are loaded"); // Throw an exception to handle the error.
